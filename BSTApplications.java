public class BSTApplications extends BinaryTree {
	...
	public int nearestNeighbour(int key) throws Exception {
		TreeNode successor = getSuccessor(key);
		TreeNode predecessor = getPredecessor(key);
		if(predecessor == null) {
	...
	public TreeNode getLCA(int x, int y) {
		if(x > y) {
			int tmp = 0;
			tmp = x;
			x = y;
